...
# List of major IDX stocks (Jakarta Stock Exchange)
IDX_STOCKS = [
    "BBCA.JK", "BBRI.JK", "BMRI.JK", "BBNI.JK", "TLKM.JK",
    "ASII.JK", "UNVR.JK", "HMSP.JK", "ICBP.JK", "KLBF.JK",
    "INDF.JK", "GGRM.JK", "SMGR.JK", "UNTR.JK", "ADRO.JK",
    "PTBA.JK", "INCO.JK", "ANTM.JK", "ITMG.JK", "TINS.JK",
    "CPIN.JK", "JPFA.JK", "MNCN.JK", "SCMA.JK", "EXCL.JK",
    "PGAS.JK", "JSMR.JK", "WIKA.JK", "PTPP.JK", "WSKT.JK",
    "AKRA.JK", "MAPI.JK", "LPPF.JK", "ACES.JK", "ITMG.JK",
    "MEDC.JK", "BSDE.JK", "PWON.JK", "CTRA.JK", "SMRA.JK",
    "ERAA.JK", "ESSA.JK", "SRIL.JK", "INTP.JK", "WSBP.JK",
    "TBIG.JK", "TPIA.JK", "SMBR.JK", "AMRT.JK", "MYOR.JK",
    "ROTI.JK", "STTP.JK", "ULTJ.JK", "BYAN.JK", "HRUM.JK",
    "ELSA.JK", "SIDO.JK", "KAEF.JK", "DVLA.JK", "PYFA.JK",
    "TOWR.JK", "ISAT.JK", "FREN.JK", "LINK.JK", "BRPT.JK",
    "AALI.JK", "LSIP.JK", "SIMP.JK", "TAPG.JK", "SMAR.JK",
    "BBTN.JK", "BJBR.JK", "BRIS.JK", "NISP.JK", "PNBN.JK",
    "MEGA.JK", "BDMN.JK", "BNLI.JK", "BNGA.JK", "BTPS.JK",
]
...
def check_consecutive_day_increase(hist, threshold=2.0, num_days=2):
    """Check if stock increased by threshold% for consecutive days"""
    if hist is None or len(hist) < (num_days + 1):
        return False, [], []
    
    # Get the last (num_days + 1) days of data
    ...
    # Calculate changes for each consecutive day
    for i in range(len(prices) - 1):
        change = ((prices[i + 1] - prices[i]) / prices[i]) * 100
        changes.append(change)
    
    # Check if all consecutive days had increase >= threshold
    all_meet_threshold = all(change >= threshold for change in changes)
...
def calculate_trading_value(hist):
    """Calculate daily trading value (Price x Volume)"""
...
def calculate_volume_trend(hist, period=5):
    """Calculate volume trend (comparing recent avg to previous avg)"""
...
def create_stock_chart(ticker, period="3mo"):
    """Create an interactive price chart with volume for a stock"""
    try:
        stock = yf.Ticker(ticker)
        hist = stock.history(period=period)
        info = stock.info
...
def process_single_stock(ticker, min_trading_value, price_threshold, num_consecutive_days=2, include_indicators=False):
    """Process a single stock and return result or error"""
    try:
        # Fetch more data to calculate technical indicators
        ...
        if hist is not None and len(hist) >= (num_consecutive_days + 1):
            # Check for consecutive days of increase
            ...
            if meets_criteria:
                ...
                # Check if trading value meets minimum
                if trading_value >= min_trading_value:
                    stock_code = ticker.replace(".JK", "")
                    stock_name = info.get('longName', stock_code) if info else stock_code
                    
                    # Build dynamic change columns
                    data_dict = {
                        'Kode': stock_code,
                        'Nama': stock_name,
                    ...
                    # Add change columns dynamically
...
def scan_stocks_with_progress(min_trading_value=15_000_000_000, price_threshold=2.0, num_consecutive_days=2, include_indicators=False):
    """Scan all stocks and filter based on criteria with progress UI"""
    filtered_stocks = []
    failed_tickers = []
    ...
    total_stocks = len(IDX_STOCKS)
    
    for idx, ticker in enumerate(IDX_STOCKS):
...
# Convert billion to actual value
min_trading_value_actual = int(min_trading_value * 1_000_000_000)

# Scan button
if st.sidebar.button("🔍 Scan Saham", type="primary"):
    st.session_state['last_scan'] = datetime.now()
    results, errors = scan_stocks_with_progress(min_trading_value_actual, price_threshold, num_consecutive_days, include_indicators)
...
st.sidebar.markdown(f"**Total Saham Dipantau:** {len(IDX_STOCKS)}")
...
if 'results' in st.session_state and not st.session_state['results'].empty:
    ...
    # Get all change columns (Kenaikan Hari -X)
    ...
    display_df = display_df.rename(columns={'Nilai Transaksi (Format)': 'Nilai Transaksi Harian'})
...
st.markdown(
    ...
    <div style='text-align: center; color: #666;'>
        <small>Data diambil dari Yahoo Finance • Update setiap 5 menit (jika auto-refresh aktif)</small>
    </div>
    """,
